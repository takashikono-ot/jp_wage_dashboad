...
layer = pdk.Layer(
    "HeatmapLayer",
    data=df_pref_map,#さっき結合して正規化したデータ
    opacity=0.4,#不透明度の設定
    get_position=["lon","lat"],#経度・緯度pdkはこの順番
    threshold=0.3,#ヒートマップの閾値
...
fig = px.scatter(df_mean_bubble,
                ...
                range_x=[150,700],
                range_y=[0,150],
                size="所定内給与額（万円）",#バブルで何を表すか？
                size_max = 38,#バブルの大きさ最大サイズ38万円
                color="年齢",
                animation_frame="集計年",#アニメーションで何を表現するか？
                animation_group="年齢")
